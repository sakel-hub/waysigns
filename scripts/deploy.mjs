...
try {
    const token = process.env.CONTENT_DB_WAYSIGNS_TOKEN ?? process.env.CONTENT_DB_TOKEN ?? argv.token
    if (!token) {
        console.error('Error: Missing ContentDB token! Provide CONTENT_DB_WAYSIGNS_TOKEN or pass --token=<token>.')
        process.exit(1)
    }
    ...
    console.log('Submitting release to ContentDB for SaKeL/waysigns:', body)

    const response = await fetch('https://content.luanti.org/api/packages/SaKeL/waysigns/releases/new/', {
        method: 'POST',
